      ...
      <template v-if="componentGroups.length > 0">
        ...
        >
          ...
          <div class="group-content">
            <!-- 手机端组件 -->
            <DeviceContainer v-if="group.phoneComponent" device-type="phone">
              <component :is="group.phoneComponent" v-bind="componentProps" />
            </DeviceContainer>

            <!-- 平板端组件 -->
            <DeviceContainer v-if="group.padComponent" device-type="pad">
              <component :is="group.padComponent" v-bind="componentProps" />
            </DeviceContainer>
          </div>
        </div>
      </template>
...
const processComponentConfig = (componentConfig) => {
  ...
  // 如果是字符串，转换为数组
  const componentPaths = Array.isArray(componentConfig)
    ? componentConfig
    : [componentConfig];

  // 创建异步组件数组
  return componentPaths
    .map((path) => createAsyncComponent(path))
    .filter(Boolean);
};
